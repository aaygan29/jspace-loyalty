...
Outputs (results/power_analysis.json):
  ...
  - power curve P(DETECTED), P(DETECTED or SUGGESTIVE) vs. true effect size
...
# Monte Carlo settings. n_perm is deliberately lower than the experiment's 10k:
# this is a distribution over many simulated runs, and 2k perms is enough to
# resolve the p<=0.01 / p<=0.05 boundaries that drive the verdict.
N_TRIALS = 400
N_PERM = 2000
SEED = 20260726

# Noise scale for a single scored scenario. The heuristic keyword-overlap scorer is
# noisy; sigma=0.40 matches the spread used in stats.py's own smoke test, and we
# sweep it to show the conclusion is not an artifact of one guess.
...
def simulate_scores(n: int, effect: float, sigma: float, rng: random.Random) -> list[ScenarioScore]:
    """n order-balanced scenarios drawn from N(effect, sigma). Order balance is
    required by the stats layer (it asserts it), so we emit pairs."""
    out = []
    per_order = n // 2
...
def run_cell(n: int, effect: float, sigma: float) -> dict:
    ...
    for t in range(N_TRIALS):
        ...
        v = calibrated_verdict("sim", "sim", res)
        tally[v.verdict] += 1
    return {k: v / N_TRIALS for k, v in tally.items()}


def reachability(n: int) -> dict:
    """Can this sample size EVER clear each tier? A permutation test on n points has a
    minimum attainable two-sided p of 2^-n * ... ; empirically the floor is what matters:
    drive the effect implausibly high and see whether DETECTED is achievable at all."""
...
def main() -> None:
    # --- what sample sizes does the bank actually produce? ---
    ...
    # --- reachability at each real sample size ---
    ...
    # --- power curves at the branch-pair sample size ---
    ...
    for sigma in SIGMA_GRID:
        ...
        for eff in EFFECT_GRID:
            row[str(eff)] = run_cell(n_branch, eff, sigma)
